...
public class HeaderPanelComponent implements Disposable {
    ...
    private final Keymap.Listener shortcutChangeListener = actionId -> {
        if (actionId.equals(RunAction.ID) || actionId.equals(StopAction.ID)) {
            unregisterShortCuts();
            registerShortCuts();
        }
    };
    ...
    public HeaderPanelComponent(TextEditor textEditor) {
        ...
        runAction = new RunAction(requestExecutor);
    ...
    private void registerShortCuts() {
        ...
        runAction.registerCustomShortcutSet(
                new CustomShortcutSet(keymap.getShortcuts(RunAction.ID)),
                editor.getComponent());

        stopAction.registerCustomShortcutSet(
                new CustomShortcutSet(keymap.getShortcuts(StopAction.ID)),
                editor.getComponent());
    }

    private void registerKeymapManagerListener() {
        KeymapManager keymapManager = KeymapManager.getInstance();
        keymapManager.addKeymapManagerListener(keymap -> {
            unregisterShortCuts();
            registerShortCuts();
        }, this);
    }
